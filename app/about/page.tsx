        ...
        <Container>
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-20">
            <motion.div initial={{ opacity: 0, x: -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.65, ease: E }}>
              ...
              <h2 className="mt-4 h3 text-white">A CRM that actually fits</h2>
                ...
                {[{ y: "2023", t: "Started", d: "First version built to solve our own lead management problem." },
                  ...
                  <div key={ev.y} className="flex gap-5">
                    <div className="flex flex-col items-center">
                      ...
                      <div className="mt-2 w-[1px] flex-1 bg-white/[0.06]" />
                    </div>
                    <div className="pb-4"><p className="text-[15px] font-bold text-white">{ev.t}</p><p className="mt-1 text-[14px] leading-relaxed text-[#737373]">{ev.d}</p></div>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      <section className="section border-t border-white/[0.06]">
        <Container>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: E }} className="mb-12">
            ...
            <h2 className="mt-4 h2 text-white">What we believe</h2>
          </motion.div>
          <div className="grid gap-5 sm:grid-cols-2">
            {VALUES.map(v => (
              <motion.div key={v.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-[10px] bg-[#FACC15]/10"><v.icon size={20} className="text-[#FACC15]" /></div>
                <h3 className="text-[17px] font-bold text-white">{v.title}</h3>
                <p className="mt-2.5 text-[15px] leading-relaxed text-[#737373]">{v.desc}</p>
              </motion.div>
            ))}
          </div>
        </Container>
