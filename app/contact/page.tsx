...
export default function ContactPage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#F8F6F3] text-[#1C1917]">
      ...
      <section id="form" className="section border-t border-[rgba(0,0,0,0.06)]">
        <Container>
          <div className="grid grid-cols-1 gap-14 lg:grid-cols-[1fr_480px] lg:gap-20">
            <motion.div initial={{ opacity: 0, x: -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.65, ease: E }}>
              ...
              <h2 className="mt-4 h3 text-[#1C1917]">Send us a message</h2>
              ...
              <div className="mt-10 space-y-5">
                {[{ t: "Response time", v: "Within 1 business day" }, { t: "Demo duration", v: "30 minutes" },
                  ...
                  <div key={r.t} className="card flex items-center justify-between px-5 py-4 rounded-[14px]">
                    <span className="text-[14px] text-[#78716C]">{r.t}</span>
                    <span className="text-[14px] font-semibold text-[#1C1917]">{r.v}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.65, ease: E }}>
              <div className="rounded-[24px] border border-[rgba(0,0,0,0.08)] bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)] p-8">
                {sent ? (
                  <div className="flex flex-col items-center py-10 text-center">
                    <CheckCircle size={48} className="text-[#C73B2A]" />
                    <h3 className="mt-5 text-[22px] font-bold text-[#1C1917]">Message sent!</h3>
                    <p className="mt-3 text-[16px] leading-relaxed text-[#78716C]">Thanks for reaching out. We'll get back to you within one business day.</p>
                    <button onClick={() => { setSent(false); setForm({ name: "", email: "", company: "", message: "", intent: "demo" }) }}
                      className="mt-7 rounded-full border border-[rgba(0,0,0,0.1)] px-6 py-2.5 text-[14px] font-medium text-[#78716C] hover:text-[#1C1917]">Send another</button>
                  </div>
                ) : (
                  <form onSubmit={submit} className="space-y-5">
                    <div>
                      <label className="text-[13px] font-semibold text-[#78716C] block mb-2">I want to...</label>
                      <div className="grid grid-cols-3 gap-2">
                        {[{ v: "demo", l: "Book a Demo" }, { v: "sales", l: "Talk Sales" }, { v: "support", l: "Get Support" }].map(opt => (
                          <button key={opt.v} type="button" onClick={() => update("intent", opt.v)}
                            className={`rounded-[10px] border py-2.5 text-[12px] font-semibold transition-all ${form.intent === opt.v ? "border-[#C73B2A]/30 bg-[#C73B2A]/[.08] text-[#C73B2A]" : "border-[rgba(0,0,0,0.08)] text-[#78716C] hover:border-[rgba(0,0,0,0.14)] hover:text-[#1C1917]"}`}>
                            {opt.l}
                          </button>
                        ))}
                      </div>
                    ...
                    {[{ k: "name", label: "Full name", type: "text", ph: "James Odhiambo", req: true },
                      ...
                      <div key={f.k}>
                        <label htmlFor={f.k} className="text-[13px] font-semibold text-[#78716C] block mb-2">{f.label}{f.req && <span className="ml-0.5 text-[#C73B2A]">*</span>}</label>
                    ...
                    <div>
                      <label htmlFor="message" className="text-[13px] font-semibold text-[#78716C] block mb-2">Message<span className="ml-0.5 text-[#C73B2A]">*</span></label>
                    ...
                    <button type="submit" disabled={busy}
                      className="flex w-full items-center justify-center gap-2 rounded-[12px] bg-gradient-to-r from-[#C73B2A] to-[#E85D3A] py-3.5 text-[15px] font-bold text-white shadow-lg shadow-[#C73B2A]/20 transition-all hover:shadow-xl disabled:opacity-60">
